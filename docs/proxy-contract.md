...
**Note on the email field.** GitHub Discussions has no per-discussion `author.email` field, and the feedback repo is public — so the email **never** lands in the public Discussion body. The Discussion's footer carries a content-neutral marker (`_A reply-to address was provided; available via the operator's inbox._` when an email was supplied, or `_Submitted anonymously._` when not). The actual email address is delivered to the operator via a private side-channel: when both `MAKE_WEBHOOK_URL` and `MAKE_API_KEY` are configured, the proxy POSTs a `{ subject, body, reply_to }` payload to a Make.com webhook after each successful Discussion. Make's scenario sends a Gmail to the operator with the user's address set as Reply-To. If those env vars are unset, the relay is skipped silently (Discussion still lands, operator just doesn't get an inbox notification).
